...
class DashboardChart(models.Model):
    ...
    kpi_model_id = fields.Many2one(
        "ir.model", string="模型 ", ondelete="set null", tracking=True
    )
    kpi_model = fields.Char(string="预览值", related="kpi_model_id.model")
    kpi_data_type = fields.Selection(
        [("count", "计数"), ("sum", "求和"), ("average", "平均值")],
        default="sum",
        string="数据类型",
        tracking=True,
    )
    kpi_measurement_field_id = fields.Many2one(
        "ir.model.fields", string="测量字段 ", ondelete="set null", tracking=True
    )
    kpi_limit_record = fields.Integer(string="记录限制", default=0)
    kpi_domain = fields.Char(string="域条件", default="[]")
    kpi_date_filter_field_id = fields.Many2one(
        "ir.model.fields",
        string="日期筛选字段",
        ondelete="set null",
    )
    meter_target = fields.Integer(string="目标值")
    kpi_date_filter_option = fields.Selection(
        [
            ...
            ("last_ninety_days", "最近90天"),
            ("last_year_days", "最近365天"),
            ("past_till_now", "过去至今"),
            ("past_excluding_today", "过去至今（不含今天）"),
            ("future_starting_today", "从今天开始的未来"),
            ("future_starting_now", "从现在开始的未来"),
            ("future_starting_tomorrow", "从明天开始的未来"),
        ],
        default="none",
        string="日期筛选选项",
        tracking=True,
    )
    kpi_include_periods = fields.Integer(string="包含周期数", default=0)
    kpi_same_period_previous_years = fields.Integer(
        string="去年同期周期数", default=0
    )
    ...
    kpi_enable_target = fields.Boolean(string="启用目标值")
    kpi_target_value = fields.Integer(string="目标值", default=0)
    ...
    model = fields.Char(string="模型引用", related="model_id.model")
    ...
    time_range = fields.Selection(
        ...
        string="按时间范围分组",
        help="在选择的分组日期选项上选择时间范围。",
    )

    map_group_by_id = fields.Many2one(
        "ir.model.fields", string="地图分组依据", tracking=True
    )
    ...
    background_color = fields.Char(string="背景颜色", default="#fff")
    is_kpi_border = fields.Boolean(string="启用边框")
    ...
    kpi_border_color = fields.Char(string="边框颜色", default="#fff")
    kpi_border_width = fields.Integer(string="边框宽度", default="10")
    font_color = fields.Char(string="字体颜色", default="#000")
    ...
    item_view_action_ids = fields.One2many(
        "item.view.action", "chart_id", string="项目操作列表"
    )
    ...
    date_filter_option = fields.Selection(
        [
            ...
            ("last_ninety_days", "最近90天"),
            ("last_year_days", "最近365天"),
            ("past_till_now", "全部历史数据"),
            ("past_excluding_today", "历史数据（不含今天）"),
            ("future_starting_today", "从今天开始的未来"),
            ("future_starting_now", "从现在开始的未来"),
            ("future_starting_tomorrow", "从明天开始的未来"),
        ],
    ...
    is_apply_multiplier = fields.Boolean(
        string="启用乘数", default=False, tracking=True
    )
    chart_multiplier_ids = fields.One2many(
        "chart.multiplier", "chart_id", string="图表乘数配置"
    )
    include_periods = fields.Integer(string="包含周期数", default=0)
    same_period_previous_years = fields.Integer(
        string="去年同期周期数", default=0
    )
    icon_option = fields.Selection(
        [("default", "默认图标"), ("custom", "自定义图标")],
        default="default",
        string="图标选项",
    )
    default_icon = fields.Char(string="默认图标名称")
    icon = fields.Binary(string="自定义图标 ")
    previous_period_comparision = fields.Boolean(
        string="上期对比",
        help="在KPI布局中启用/禁用上期对比功能。",
        tracking=True,
    )
    previous_period_duration = fields.Integer(
        ...
        default=1,
        tracking=True,
        help="设置与配置的日期筛选器进行对比的整数值。\n"
             "例如：日期筛选器设置为'今年'，如果此值设置为2，"
             "则'今年'的数据将与2年前的数据进行对比。",
    )
    previous_period_type = fields.Selection(
        [("percentage", "百分比"), ("value", "数值")],
        string="上期对比类型",
        default="percentage",
        tracking=True,
    )
    show_unit = fields.Boolean("显示单位", help="在坐标轴上设置单位类型。")
    unit_type = fields.Selection(
        [("monetary", "货币"), ("custom", "自定义")],
        string="单位类型",
        default="monetary",
        help="为坐标轴选择单位类型。",
    )
    ...
    button_color = fields.Char(string="顶部按钮颜色")
    ...
    hide_false_value = fields.Boolean(
        string="隐藏假值",
        default=True
    )
    ...
    def get_tile_data(self, conf_obj, previous=0):
        ...
        if not conf_obj.model:
            return {"type": "error", "message": "请选择模型！"}
