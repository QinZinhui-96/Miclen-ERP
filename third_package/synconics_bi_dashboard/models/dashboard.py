...
class Dashboard(models.Model):
    ...
    menu_active = fields.Boolean(
        string="菜单激活",
        default=True
    )
    grid_stack_dimensions = fields.Json(
        string="网格堆叠尺寸", default=[], copy=False
    )
...
class DashboardMail(models.Model):
    ...
    dashboard_id = fields.Many2one("dashboard.dashboard", string="仪表盘")
    ...
    chart_ids = fields.Many2many("dashboard.chart", string="图表", required=True)
    recipient_ids = fields.Many2many("res.partner", string="收件人", required=True)
    is_automated = fields.Boolean(string="自动发送邮件")
