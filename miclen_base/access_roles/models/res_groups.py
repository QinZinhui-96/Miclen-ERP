...
class ResGroups(models.Model):
    ...
    def get_groups_by_application(self):
        """Return all groups classified by privilege category, as a list:
                [(category, kind, groups, category_name), ...],
            where ``category`` and ``groups`` are recordsets, and ``kind`` is either
            ``'boolean'`` or ``'selection'``. Categories are given in sequence
            order. If ``kind`` is ``'selection'``, ``groups`` are given in
            reverse implication order.
        """
        def linearize(category, gs, category_name):
            if category and category.xml_id == 'base.module_category_user_type':
                return (category, 'selection', gs.sorted('id'), category_name)
            order = {g: len(g.all_implied_ids & gs) for g in gs}
            if category and category.xml_id == 'base.module_category_accounting_accounting':
                return (category, 'selection', gs.sorted(key=order.get), category_name)
            if len(set(order.values())) == len(gs):
                return (category, 'selection', gs.sorted(key=order.get), category_name)
            else:
                return (category or self.env['ir.module.category'], 'boolean', gs, (100, 'Other'))

        by_category, others = defaultdict(self.browse), self.browse()
        for g in self.get_application_groups([]):
            if g.privilege_id and g.privilege_id.category_id:
                by_category[g.privilege_id.category_id] += g
            else:
                others += g
        res = []
        for category, gs in sorted(by_category.items(), key=lambda it: it[0].sequence or 0):
            res.append(linearize(category, gs, (category.sequence, category.name)))
        if others:
            res.append(
    ...
    def _update_role_groups_view(self):
        ...
        model_name = self.env.context.get('model', 'access.role')
        if self.env.context.get('install_filename') or self.env.context.get(
            ...
            invisible_information = (
                ...
                "The implied groups are calculated using these values.")
            for category, kind, gs, category_name in sorted_tuples:
                attrs = {}
                if kind == 'selection':
                    ...
                    category_name_str = category.name if category else 'Other'
                    xml4.append(E.separator(string=category_name_str, **attrs))
                    left_group, right_group = [], []
                    group_count = 0
                    for g in gs:
                        field_name = self.name_boolean_group(g.id)
                        dest_group = left_group if group_count % 2 == 0 else right_group
                        # Use the English full group name (Category / Role) so that
                        # translations cannot truncate the category prefix and make
                        # all groups look identical (e.g. all showing as "User").
                        attrs = {'string': g.with_context(lang='en_US').name}
                        attrs = self.customize_role_group_fields(field_name, attrs,
                                                                 model_name)
                        if g == group_no_one:
                            dest_group.append(
